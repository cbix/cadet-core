 ...
 * @details Unit operation index does not need to match index of unit operation
 * in an array.
 * @param [in] models List of models
 * @param [in] unitOpIdx Unit operation index to look for
 * @return Array index of the unit operation identified by unit operation index
 * or invalid array index if the unit operation has not been found
 */
inline unsigned int
indexOfUnitOp(const std::vector<cadet::IUnitOperation *> &models,
              unsigned int unitOpIdx) {
...
/**
 * @brief Returns whether a given unit operation is a terminal node in the
 * network
 ...
 * @return @c true if the unit operation is a terminal node in the network, @c
 * false otherwise
 */
inline bool isTerminal(int const *const conn, unsigned int size,
                       int unitOpIdx) {
 ...
 * @return @c true if the list contains at least one unit operation model with
 * multiple ports, @c false otherwise
 */
inline bool
hasMultiPortUnits(const std::vector<cadet::IUnitOperation *> &models) {
...
ModelSystem::ModelSystem()
    : _jacNF(nullptr), _jacFN(nullptr), _jacActiveFN(nullptr),
      _curSwitchIndex(0), _tempState(nullptr), _initState(0, 0.0),
      _initStateDot(0, 0.0) {}
...
void ModelSystem::addModel(IModel *unitOp) {
  ...
  if (indexOfUnitOp(_models, unitOp->unitOperationId()) < _models.size())
    throw InvalidParameterException(
        "Cannot add model because of already existing unit operation id " +
...
IUnitOperation const *
ModelSystem::getUnitOperationModel(unsigned int unitOpIdx) const {
  for (IUnitOperation *m : _models) {
    if (m->unitOperationId() == unitOpIdx)
      return m;
  }
...
void ModelSystem::removeModel(IModel const *unitOp) {
  std::vector<IUnitOperation *>::iterator it =
      std::find(_models.begin(), _models.end(),
                static_cast<IUnitOperation const *>(unitOp));
  if (it != _models.end())
    _models.erase(it);
}

IModel *ModelSystem::removeModel(UnitOpIdx unitOp) {
  for (std::vector<IUnitOperation *>::iterator it = _models.begin();
       it != _models.end(); ++it) {
...
IExternalFunction const *
ModelSystem::getExternalFunction(unsigned int index) const {
  if (index < _extFunctions.size())
    return _extFunctions[index];
  else
    return nullptr;
...
void ModelSystem::removeExternalFunction(IExternalFunction const *extFun) {
  for (std::vector<IExternalFunction *>::iterator it = _extFunctions.begin();
       it != _extFunctions.end(); ++it) {
    if (*it == extFun) {
...
ModelSystem::getModelStateOffsets(UnitOpIdx unitOp) const CADET_NOEXCEPT {
...
/**
 * @brief Create data structures to keep track of entries and locations in the
 * state vector
 * @details Three data structures are created. One keeps track of the offsets to
 * each unit operation. The second keeps track of the size of each unit
 * operation. The third is mapping from unit operation and component index to
 * unique inlet DOF index.
 */
void ModelSystem::rebuildInternalDataStructures() {
  ...
  /*
          A mapping is required that turns a local model, port index, and
     component index into the location of the inlet DOF in the global state
     vector. Some unit operations do not have inlet DOFs (e.g., inlet unit
     operation). Hence, a map is constructed which converts local indices into
     global ones.
  */
 ...
 * be allocated for the coupling matrices. This function walks the connections
 * over the entire simulation in order to determine the maximum number of
 *          connections over the whole simulation which governs the number of
 * entries in the sparse superstructure coupling matrices. Finally, the required
 * memory is allocated in the matrices.
 */
void ModelSystem::allocateSuperStructMatrices() {
  // Step 1: Calculate number of connections per unit operation per valve switch
  // We record the number of outgoing connections (i.e., components) which act
  // as sources in the bottom macro-row of the superstructure
  const unsigned int nModels = numModels();
  std::vector<unsigned int> sourcesPerUnitOpPerConfig(
      nModels * _switchSectionIndex.size(), 0u);

  for (std::size_t idx = 0; idx < _switchSectionIndex.size(); ++idx) {
    int const *ptrConn = _connections[idx];

    for (unsigned int i = 0; i < _connections.sliceSize(idx) / 6;
         ++i, ptrConn += 6) {
  ...
  // Step 2: Take maximum over valve switches to obtain maximum number of matrix
  // entries per unit operation
  std::vector<unsigned int> numOutgoing(nModels, 0u);

  // Assign the first row of sourcesPerUnitOpPerConfig to the maximum number of
  // outputs
  std::copy(sourcesPerUnitOpPerConfig.begin(),
            sourcesPerUnitOpPerConfig.begin() + nModels, numOutgoing.begin());

  // Loop over remaining lines and take maximum per element
  for (std::size_t sectionIdx = 1; sectionIdx < _switchSectionIndex.size();
       ++sectionIdx) {
    for (unsigned int i = 0; i < nModels; ++i) {
      numOutgoing[i] = std::max(
          numOutgoing[i], sourcesPerUnitOpPerConfig[nModels * sectionIdx + i]);
    }
  }

  // Step 3: Allocate memory based on maximum number of connections for each
  // unit operation
  for (unsigned int i = 0; i < numModels(); ++i) {
    // Bottom macro-row
    ...
    // Right macro-column
    // Each unit operation has inlets equal to numComponents * numInletPorts so
    // long as the unit operation has an inlet
    IUnitOperation const *const model = _models[i];
    if (model->hasInlet())
...
bool ModelSystem::configureModelDiscretization(
    IParameterProvider &paramProvider, const IConfigHelper &helper) {
  ...
  // Calculate the sizes that need to be allocated for all the inlets and
  // outlets
  ...
  if (paramProvider.exists("external")) {
    ...
    oss << "source_" << std::setfill('0') << std::setw(3)
  ...
  _gmres.initialize(numCouplingDOF(), maxKrylov,
                    linalg::toOrthogonalization(gsType), maxRestarts);
  ...
  LOG(Debug) << "Num units " << _models.size() << " max ID "
             << maxUnitOperationId() << " total DOF " << numDofs();
  LOG(Debug) << "Unit op state vector size: uoDOFs = "
             << log::VectorPtr<unsigned int>(_dofs.data(), _dofs.size());
  LOG(Debug) << "Unit op state vector offsets: uoOffset = "
             << log::VectorPtr<unsigned int>(_dofOffset.data(),
                                             _dofOffset.size());
...
void ModelSystem::configureSwitches(IParameterProvider &paramProvider) {
  ...
  _connections.clear();
  _connections.reserve(numSwitches * 6 * _models.size() * _models.size(),
                       numSwitches);
  _flowRates.clear();
  _flowRates.reserve(numSwitches * _models.size() * _models.size(),
                     numSwitches);
  ...
  // If we have unit operations with multiple ports, we require ports in
  // connection list
  ...
  if (paramProvider.exists("CONNECTIONS_INCLUDE_DYNAMIC_FLOW"))
    _hasDynamicFlowRates =
        paramProvider.getBool("CONNECTIONS_INCLUDE_DYNAMIC_FLOW");

  std::ostringstream oss;
  for (unsigned int i = 0; i < numSwitches; ++i) {
    oss.str("");
    oss << "switch_" << std::setfill('0') << std::setw(3)
        << std::setprecision(0) << i;
    ...
    if ((i > 0) && (_switchSectionIndex.back() <=
                    _switchSectionIndex[_switchSectionIndex.size() - 2]))
      throw InvalidParameterException(
          "SECTION index has to be monotonically increasing (switch " +
          std::to_string(i) + ")");
    ...
    if (!conListHasPorts) {
      if (_hasDynamicFlowRates) {
        if ((connFlow.size() % 8) != 0)
          throw InvalidParameterException(
              "CONNECTIONS matrix has to have 8 columns if "
              "CONNECTIONS_INCLUDE_PORTS is disabled and "
              "CONNECTIONS_INCLUDE_DYNAMIC_FLOW is enabled");
      } else {
        if ((connFlow.size() % 5) != 0)
          throw InvalidParameterException(
              "CONNECTIONS matrix has to have 5 columns if "
              "CONNECTIONS_INCLUDE_PORTS is disabled and "
    ...
    if (!_hasDynamicFlowRates) {
      if ((connFlow.size() % 7) != 0)
        throw InvalidParameterException(
            "CONNECTIONS matrix has to have 5 or 7 columns if "
            "CONNECTIONS_INCLUDE_DYNAMIC_FLOW is disabled");
    ...
    if ((connFlow.size() % 10) != 0)
      throw InvalidParameterException(
          "CONNECTIONS matrix has to have 10 columns");
    ...
    if (fr.size() > 0) {
      ...
      _parameters[makeParamId(flowHash, UnitOpIndep, conn[2], conn[3], conn[0],
                              conn[1], _switchSectionIndex.back())] =
          _flowRates.back();
      _parameters[makeParamId(flowHashLin, UnitOpIndep, conn[2], conn[3],
                              conn[0], conn[1], _switchSectionIndex.back())] =
          _flowRatesLin.back();
      _parameters[makeParamId(flowHashQuad, UnitOpIndep, conn[2], conn[3],
                              conn[0], conn[1], _switchSectionIndex.back())] =
          _flowRatesQuad.back();
      _parameters[makeParamId(flowHashCub, UnitOpIndep, conn[2], conn[3],
                              conn[0], conn[1], _switchSectionIndex.back())] =
          _flowRatesCub.back();
      for (std::size_t j = 1; j < fr.size(); ++j) {
        ...
        // Check if a previous identical connection (except for component
        // indices) exists
        bool found = false;
        for (unsigned int k = 0; k < j; ++k) {
          if ((conn[6 * k] == conn[6 * j]) &&
              (conn[6 * k + 1] == conn[6 * j + 1]) &&
              (conn[6 * k + 2] == conn[6 * j + 2]) &&
              (conn[6 * k + 3] == conn[6 * j + 3])) {
            found = true;
            break;
          }
        }
        ...
        if (!found) {
          _parameters[makeParamId(flowHash, UnitOpIndep, conn[6 * j + 2],
                                  conn[6 * j + 3], conn[6 * j], conn[6 * j + 1],
                                  _switchSectionIndex.back())] =
              (_flowRates.back() + j);
          _parameters[makeParamId(flowHashLin, UnitOpIndep, conn[6 * j + 2],
                                  conn[6 * j + 3], conn[6 * j], conn[6 * j + 1],
                                  _switchSectionIndex.back())] =
              (_flowRatesLin.back() + j);
          _parameters[makeParamId(flowHashQuad, UnitOpIndep, conn[6 * j + 2],
                                  conn[6 * j + 3], conn[6 * j], conn[6 * j + 1],
                                  _switchSectionIndex.back())] =
              (_flowRatesQuad.back() + j);
          _parameters[makeParamId(flowHashCub, UnitOpIndep, conn[6 * j + 2],
                                  conn[6 * j + 3], conn[6 * j], conn[6 * j + 1],
                                  _switchSectionIndex.back())] =
              (_flowRatesCub.back() + j);
        }
      }
    } else {
      // Add empty slice
    ...
    if (_linearSolutionMode == 1) {
      ...
      const util::SlicedVector<int> adjList =
          graph::adjacencyListFromConnectionList(conn.data(), _models.size(),
                                                 conn.size() / 6);
      ...
      // Simple heuristic: At least 25 models (regardless of complexity) =>
      // Parallelize
      if (_models.size() >= 25) {
        _linearModelOrdering.pushBackSlice(0);
        LOG(Debug) << "Select parallel solution method for switch " << i
  ...
  if (_switchSectionIndex[0] != 0)
    throw InvalidParameterException(
        "First element of SECTION in connections group has to be 0");
}

/**
 * @brief Add default ports to connection list
 * @details Adds source and destination ports of @c -1 to the connection list.
 ...
 */
void ModelSystem::addDefaultPortsToConnectionList(
    std::vector<double> &conList) const {
 ...
 *          The list is overwritten, that is, all pointers and iterators are
 * invalidated.
 * @param [in,out] conList On entry, list of connections without dynamic flow
 * rates; on exit, list of connections including dynamic flow rates
 */
void ModelSystem::addDefaultDynamicFlowRatesToConnectionList(
    std::vector<double> &conList) const {
...
bool ModelSystem::configureModel(IParameterProvider &paramProvider,
                                 unsigned int unitOpIdx) {
 ...
 * @details Throws an exception if something is incorrect. Reformats the
 * connection list by substituting unit operation IDs with local indices.
 * @param [in] conn Matrix with 7 columns holding all connections. The matrix is
 * expected to be in row-major storage format.
 * @param [out] connOnly Matrix with 6 columns holding all connections. While @p
 * conn contains connection indices and flow rate, this matrix only holds
 * connection indices. It is assumed to be pre-allocated (same number of rows as
 * @p conn). The unit operation IDs are substituted by the corresponding indices
 * of the unit operations in the local _models vector.
 * @param [out] flowRates Vector with flow rates for each connection in the
 * list. It is assumed to be pre-allocated (same number of rows as @p conn).
 * @param [out] flowRatesLin Vector with linear flow rate coefficients for each
 * connection in the list. It is assumed to be pre-allocated (same number of
 * rows as @p conn).
 * @param [out] flowRatesQuad Vector with quadratic flow rate coefficients for
 * each connection in the list. It is assumed to be pre-allocated (same number
 * of rows as @p conn).
 * @param [out] flowRatesCub Vector with cubic flow rate coefficients for each
 * connection in the list. It is assumed to be pre-allocated (same number of
 * rows as @p conn).
 * @param [in] idxSwitch Index of the valve switch that corresponds to this
 * connection list
 */
void ModelSystem::checkConnectionList(const std::vector<double> &conn,
                                      std::vector<int> &connOnly,
                                      std::vector<double> &flowRates,
                                      std::vector<double> &flowRatesLin,
                                      std::vector<double> &flowRatesQuad,
                                      std::vector<double> &flowRatesCub,
  ...
  for (std::size_t i = 0; i < conn.size() / 10; ++i) {
    ...
    if (uoDest < 0)
      throw InvalidParameterException("In CONNECTIONS matrix (switch " +
                                      std::to_string(idxSwitch) + " row " +
                                      std::to_string(i) +
                                      "): Destination unit operation id has to "
                                      "be at least 0 in connection");
    ...
    if (static_cast<unsigned int>(uoSource) >= _models.size())
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Source unit operation id " +
          std::to_string(uoSource) + " not found in connection");
    if (static_cast<unsigned int>(uoDest) >= _models.size())
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Destination unit operation id " +
    ...
    if (!_models[uoSource]->hasOutlet())
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Source unit operation " +
    ...
    // Check port indices
    if ((portSource >= 0) && (static_cast<unsigned int>(portSource) >=
                              _models[uoSource]->numOutletPorts()))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Source port index (" +
          std::to_string(portSource) + ") exceeds number of outlet ports (" +
          std::to_string(_models[uoSource]->numOutletPorts()) + ")");
    if ((portDest >= 0) && (static_cast<unsigned int>(portDest) >=
                            _models[uoDest]->numInletPorts()))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Destination port index (" +
          std::to_string(portDest) + ") exceeds number of inlet ports (" +
          std::to_string(_models[uoDest]->numInletPorts()) + ")");

    if (((portSource < 0) && (portDest >= 0)) ||
        ((portSource >= 0) && (portDest < 0)))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) +
          "): Only source or destination (not both) are set to connect all "
          "ports in connection from unit " +
    ...
    if ((compSource >= 0) && (static_cast<unsigned int>(compSource) >=
                              _models[uoSource]->numComponents()))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Source component index (" +
          std::to_string(compSource) + ") exceeds number of components (" +
          std::to_string(_models[uoSource]->numComponents()) + ")");
    if ((compDest >= 0) && (static_cast<unsigned int>(compDest) >=
                            _models[uoDest]->numComponents()))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) + "): Destination component index (" +
          std::to_string(compDest) + ") exceeds number of components (" +
          std::to_string(_models[uoDest]->numComponents()) + ")");

    if (((compSource < 0) && (compDest >= 0)) ||
        ((compSource >= 0) && (compDest < 0)))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) +
          "): Only source or destination (not both) are set to connect all "
          "components in connection from unit " +
          std::to_string(_models[uoSource]->unitOperationId()) + " to " +
          std::to_string(_models[uoDest]->unitOperationId()));
    ...
    if ((compSource < 0) && (compDest < 0) &&
      ...
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          " row " + std::to_string(i) +
          "): Number of components not equal when connecting all components "
          "from unit " +
          std::to_string(_models[uoSource]->unitOperationId()) + " to " +
          std::to_string(_models[uoDest]->unitOperationId()));
    ...
    // Check if such a connection has occurred before (for a different
    // component)
    bool found = false;
    for (unsigned int j = 0; j < i; ++j) {
      if ((conn[10 * j] == uoSource) && (uoDest == conn[10 * j + 1]) &&
          (conn[10 * j + 2] == portSource) && (portDest == conn[10 * j + 3])) {
  ...
  for (std::size_t i = 0; i < _models.size(); ++i) {
    // Unit operations with only one port (inlet or outlet) do not need to
    // balance their flows
    if ((totalInflow[i] >= 0.0) && (totalOutflow[i] == 0.0) &&
        _models[i]->hasInlet() && !_models[i]->hasOutlet())
      continue;
    if ((totalInflow[i] == 0.0) && (totalOutflow[i] >= 0.0) &&
        !_models[i]->hasInlet() && _models[i]->hasOutlet())
      continue;
    ...
    if ((totalOutflow[i] >= 0.0) &&
        isTerminal(connOnly.data(), connOnly.size() / 6, i))
      continue;
    ...
    if ((diffLin >= 1e-15) || (diffLin > 1e-15 * std::abs(totalOutflowLin[i])))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          "): Linear flow rate coefficient balance is not closed for unit "
          "operation " +
          std::to_string(i) + ", imbalanced by " + toSciString(diffLin));

    const double diffQuad = std::abs(totalInflowQuad[i] - totalOutflowQuad[i]);
    if ((diffQuad >= 1e-15) ||
        (diffQuad > 1e-15 * std::abs(totalOutflowQuad[i])))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          "): Quadratic flow rate coefficient balance is not closed for unit "
          "operation " +
          std::to_string(i) + ", imbalanced by " + toSciString(diffQuad));

    const double diffCub = std::abs(totalInflowCub[i] - totalOutflowCub[i]);
    if ((diffCub >= 1e-15) || (diffCub > 1e-15 * std::abs(totalOutflowCub[i])))
      throw InvalidParameterException(
          "In CONNECTIONS matrix (switch " + std::to_string(idxSwitch) +
          "): Cubic flow rate coefficient balance is not closed for unit "
          "operation " +
...
std::unordered_map<ParameterId, double>
ModelSystem::getAllParameterValues() const {
  std::unordered_map<ParameterId, double> data;
  std::transform(
      _parameters.begin(), _parameters.end(), std::inserter(data, data.end()),
      [](const std::pair<const ParameterId, active *> &p) {
        return std::make_pair(p.first, static_cast<double>(*p.second));
      });

  for (IUnitOperation *m : _models) {
    const std::unordered_map<ParameterId, double> localData =
        m->getAllParameterValues();
...
bool ModelSystem::hasParameter(const ParameterId &pId) const {
  for (IUnitOperation *m : _models) {
    if ((m->unitOperationId() == pId.unitOperation) ||
        (pId.unitOperation == UnitOpIndep)) {
...
double ModelSystem::getParameterDouble(const ParameterId &pId) const {
  ...
  const std::unordered_map<ParameterId, active *>::const_iterator it =
      _parameters.find(pId);
...
bool ModelSystem::setParameterImpl(const ParameterId &pId,
...
bool ModelSystem::setParameter(const ParameterId &pId, double value) {
  ...
  if (paramHandle != _parameters.end()) {
    ...
    if (flowHash == pId.name) {
      ...
      const auto it = std::find(_switchSectionIndex.begin(),
                                _switchSectionIndex.end(), pId.section);
      if (it != _switchSectionIndex.end()) {
        const unsigned int idxSwitch =
            std::distance(_switchSectionIndex.begin(), it);
        int const *ptrConn = _connections[idxSwitch];
        active *conRates = _flowRates[idxSwitch];

        // Set value for all flow rates of the same connection (except for
        // components)
        for (unsigned int i = 0; i < _connections.sliceSize(idxSwitch) / 6;
             ++i, ptrConn += 6, ++conRates) {
          if ((ptrConn[2] != pId.component) ||
              (ptrConn[3] != pId.particleType) ||
...
void ModelSystem::setSensitiveParameterValue(const ParameterId &pId,
                                             double value) {
  if (pId.unitOperation == UnitOpIndep) {
    // Handle flow rates
    auto paramHandle = _parameters.find(pId);
    if ((paramHandle != _parameters.end()) &&
        contains(_sensParams, paramHandle->second)) {
      paramHandle->second->setValue(value);
...
bool ModelSystem::setSensitiveParameter(const ParameterId &pId,
  ...
  if (pId.unitOperation == UnitOpIndep) {
    auto paramHandle = _parameters.find(pId);
    if (paramHandle != _parameters.end()) {
      LOG(Debug) << "Found parameter " << pId << " in ModelSystem: Dir "
                 << adDirection << " is set to " << adValue;
      ...
      if (flowHash == pId.name) {
        ...
        if (it != _switchSectionIndex.end()) {
          ...
          // Set AD direction for all flow rates of the same connection (except
          // for components)
          for (unsigned int i = 0; i < _connections.sliceSize(idxSwitch) / 6;
               ++i, ptrConn += 6, ++conRates) {
            if ((ptrConn[2] != pId.component) ||
...
void ModelSystem::reportSolution(ISolutionRecorder &recorder,
                                 double const *const solution) const {
  for (std::size_t i = 0; i < _models.size(); ++i) {
...
unsigned int ModelSystem::requiredADdirs() const CADET_NOEXCEPT {
  // Take maximum of required AD directions since each unit operation is
  // (locally) independent from the rest
  unsigned int dirs = 0;
  for (IUnitOperation *m : _models)
...
void ModelSystem::setSectionTimes(double const *secTimes,
 ...
 * @details ModelSystem uses additional DOFs to decouple a system of unit
 * operations for parallelization. These additional DOFs don't get an error
 * tolerance from the user because he shouldn't be aware of those
 * (implementation detail). This function is responsible for calculating error
 *          tolerances for these additional coupling DOFs.
 *
 * @param [in] errorTol Pointer to array of error tolerances for system without
 * coupling DOFs
 * @param [in] errorTolLength Length of @p errorTol array
 * @return Vector with error tolerances for additional coupling DOFs
 */
std::vector<double>
ModelSystem::calculateErrorTolsForAdditionalDofs(double const *errorTol,
                                                 unsigned int errorTolLength) {
  // Return empty vector since we don't have coupling DOFs, yet
  return std::vector<double>(0, 0.0);
}

void ModelSystem::expandErrorTol(double const *errorSpec,
